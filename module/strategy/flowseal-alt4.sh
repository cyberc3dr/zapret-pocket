...
config="--filter-tcp=80,443 --hostlist=$MODPATH/list/google.txt --hostlist-exclude=$MODPATH/list/exclude.txt --ip-id=zero --dpi-desync=fake,multisplit --dpi-desync-repeats=6 --dpi-desync-fooling=badseq --dpi-desync-badseq-increment=1000 --dpi-desync-fake-tls=$MODPATH/fake/tls_clienthello_www_google_com.bin --new"
