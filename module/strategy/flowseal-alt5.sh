...
config="--filter-tcp=80,443 --hostlist=$MODPATH/list/google.txt --hostlist-exclude=$MODPATH/list/exclude.txt --ip-id=zero --dpi-desync=syndata,multidisorder --new"
config="$config --filter-tcp=80,443 --hostlist=$MODPATH/list/default.txt --hostlist=$MODPATH/list/reestr.txt --hostlist=$MODPATH/list/custom.txt --hostlist-exclude=$MODPATH/list/exclude.txt --dpi-desync=syndata,multidisorder --new"
config="$config --filter-tcp=80,443 --ipset=$MODPATH/ipset/ipset-v4.txt --ipset=$MODPATH/ipset/ipset-v6.txt --ipset=$MODPATH/ipset/custom.txt --ipset-exclude=$MODPATH/ipset/exclude.txt --dpi-desync=syndata,multidisorder --dup=2 --dup-cutoff=n3 --new"
...
if [ "$(cat "$MODPATH/config/bypass-calls" 2>/dev/null || echo 0)" = "1" ]; then
   config="$config --filter-tcp=2053,2083,2087,2096,8443 --hostlist-domains=discord.media --dpi-desync=syndata,multidisorder --new"
